...
class TailorTalkAgent:
    ...
    def _send_with_retry(self, chat, message, max_attempts=3):
        """Send a message to Gemini, retrying on transient 503s and falling
        back to a secondary model if the primary stays overloaded."""
        last_error = None
        ...
        # Primary model still failing — retry once on a fallback model
        ...
        if fallback_model:
            try:
                ...
                return fallback_chat.send_message(message)
            except ServerError:
                pass
